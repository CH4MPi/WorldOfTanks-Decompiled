...
class EquipmentsController(MethodsRules, IBattleController):
    __slots__ = ('__eManager', '__arena', '_order', '_equipments', '__preferredPosition', '__equipmentCount', 'onEquipmentAdded', 'onEquipmentUpdated', 'onEquipmentMarkerShown', 'onEquipmentCooldownInPercent', 'onEquipmentCooldownTime', 'onCombatEquipmentUsed', 'onEquipmentReset', 'onClearEquipments')
    ...
    def __init__(self, setup):
        ...
        self.onClearEquipments = Event.Event(self.__eManager)
    ...
    def getOrderedEquipmentsLayout(self):
        return map(lambda intCD: (intCD, self._equipments.get(intCD)), self._order)
    ...
    def setEquipment(self, intCD, quantity, stage, timeRemaining, totalTime):
        if timeRemaining == -1 and totalTime == -1 and stage == EQUIPMENT_STAGES.NOT_IN_ARENA:
            return
        else:
